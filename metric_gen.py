...
def stack_process(pred, bilinear, gt, offset_frames=0):
    ...
    for i in range(frames):
        ...
        x1 = np.array(stack_pred).astype(np.float64)
        x2 = np.array(stack_bilinear).astype(np.float64)
        y = np.array(stack_gt).astype(np.float64)
    ...
    #tifffile.imsave(str(exp_dir/f"{stem}_GTnormtopred.tif"), np.stack(y_norm1s).astype(np.float64))
    #tifffile.imsave(str(exp_dir/f"{stem}_prednorm.tif"), np.stack(x1_norms).astype(np.float64))
    #tifffile.imsave(str(exp_dir/f"{stem}_GTnormtobilinear.tif"), np.stack(y_norm2s).astype(np.float64))
    #tifffile.imsave(str(exp_dir/f"{stem}_bilinearnorm.tif"), np.stack(x2_norms).astype(np.float64))
