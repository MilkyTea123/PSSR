...
def unet_image_from_tiles_partialsave(learn, in_img, tile_sz=(256, 256), scale=(4, 4), overlap_pct=(0.50, 0.50), img_info=None):
    ...
    return assembled.astype(np.float64)
...
def unet_image_from_tiles_blend(learn, in_img, use_tiles, tile_sz=256, scale=4, overlap_pct=5.0, img_info=None):
    ...
    return assembled.astype(np.float64).clip(0.,1.)
...
def tif_predict_movie(learn,
    ...
    for i in range(times):
        im.seek(i)
        imgs.append(np.array(im).astype(np.float64) / 255.)
...
def czi_predict_movie(learn,
    ...
    with czifile.CziFile(czi_in) as czi_f:
        ...
        data = czi_f.asarray().astype(np.float64) / 255.
        preds = []
        origs = []
...
def draw_random_tile(img_data, tile_sz, thresh, thresh_pct):
    ...
    while not found_tile:
        tile, (xs,ys) = draw_tile(img_data, tile_sz).astype(np.uint32)
    ...
    return PIL.Image.fromarray(tile, mode='I'), box
